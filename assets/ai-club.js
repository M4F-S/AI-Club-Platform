(function () {
  ...
  async function updateNavAuth() {
    ...
    // Clean up any previously injected auth items
    links.querySelectorAll('.nav-auth-item').forEach(el => el.remove());

    const isLoggedIn = ok && data.user;

    // Hide [data-auth-hide] links (Member Login, Join us) when logged in
    document.querySelectorAll('[data-auth-hide]').forEach(el => {
      el.closest('li').style.display = isLoggedIn ? 'none' : '';
    });
